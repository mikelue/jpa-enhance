...
/**
 * 兩階段分頁的執行 IoC 類別，主要責任在依傳入的 {@link FetchPhase} 參數，決定是否要執行兩階段查詢.
 *
 * <p>會依資料內容，設定查詢的資料筆數，是用來設定查出的結果，
 * 以調整真正分頁的情況({@link PagingResultBean#setResultSize(int)})，分為下列兩種策略:</p>
 * <ol>
 * 		<li>從「開始筆數」到「結束筆數」</li>
 * 		<li>從「第一筆資料」到「開始筆數 - 1」</li>
 * </ol>
 * <p>在上述的兩種策略，用途在第一個查詢(第一個策略)沒有資料時，重新執行第二次查詢(第二個策略)，
